...
static void dwin_on_cmd_tool(uint16_t tval) {
  switch (tval) {
    ...
    case VARVAL_TOOL_ZUP:
      new_zoffset_up = zprobe_zoffset + planner.steps_to_mm[Z_AXIS] * 10;
      //SERIAL_ECHOPGM("Z up");
      //SERIAL_ECHO((float)new_zoffset_up);
          if (WITHIN(new_zoffset_up, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX)) {
            thermalManager.babystep_axis(Z_AXIS, 10);
            zprobe_zoffset = new_zoffset_up;
          }
      break;

    case VARVAL_TOOL_ZDOWN:
      new_zoffset_down = zprobe_zoffset + planner.steps_to_mm[Z_AXIS] * -10;
      //SERIAL_ECHOPGM("Z down");
      //SERIAL_ECHO((float)new_zoffset_down);
          if (WITHIN(new_zoffset_down, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX)) {
            thermalManager.babystep_axis(Z_AXIS, -10);
            zprobe_zoffset = new_zoffset_down;
          }
      break;
    #endif
      
    case VARVAL_RGBLED_ON:
      enqueue_and_echo_commands_P(PSTR("M150 R U B P255"));
      break;

    case VARVAL_RGBLED_OFF:
      enqueue_and_echo_commands_P(PSTR("M150 P0"));
      break;

    case VARVAL_RGB_PARAM_APPLY:
     /*myFysTLcd.ftCmdStart(VARADDR_APPLY_TUNE_RGBLED);
      if (myFysTLcd.ftCmdReceive(6)) {
        int r, g, b;
        myFysTLcd.ftCmdGetI16(r);
        myFysTLcd.ftCmdGetI16(g);
        myFysTLcd.ftCmdGetI16(b);
        char buffRGB[25];
        sprintf_P(buffRGB, PSTR("M150 R%d U%d B%d P255"), r, g, b);
        SERIAL_ECHOLNPAIR("contents of buffer:", buffRGB);
        enqueue_and_echo_commands_P(buffRGB);
      }*/
      break;
    case  VARVAL_RGBLED_RED:
      enqueue_and_echo_commands_P(PSTR("M150 R255 U0 B0 P128"));
      break;
    case  VARVAL_RGBLED_ORANGE:
      enqueue_and_echo_commands_P(PSTR("M150 R255 U80 B0 P128"));
      break;
    case  VARVAL_RGBLED_YELLOW:
      enqueue_and_echo_commands_P(PSTR("M150 R255 U255 B0 P128"));
      break;
    case  VARVAL_RGBLED_GREEN:
      enqueue_and_echo_commands_P(PSTR("M150 R0 U255 B0 P128"));
      break;
    case  VARVAL_RGBLED_BLUE:
      enqueue_and_echo_commands_P(PSTR("M150 R0 U0 B255 P128"));
      break;
    case  VARVAL_RGBLED_INDIGO:
      enqueue_and_echo_commands_P(PSTR("M150 R0 U255 B255 P128"));
      break; 
    case  VARVAL_RGBLED_PURPLE:
      enqueue_and_echo_commands_P(PSTR("M150 R255 U0 B255 P128"));
      break;
    case  VARVAL_RGBLED_0:
      enqueue_and_echo_commands_P(PSTR("M150 P0"));
      break;
    case  VARVAL_RGBLED_50:
      enqueue_and_echo_commands_P(PSTR("M150 R U B P128"));
      break; 
    case  VARVAL_RGBLED_100:
      enqueue_and_echo_commands_P(PSTR("M150 R U B P255"));
      break;                      
    // END
...
static void dwin_on_cmd(millis_t& tNow) {
  ...
  // ADD F.H.
case VARADDR_BABYSTEP:
      dwin_on_cmd_tool(tval);
      break; 

case VARADDR_PARAM_RGBLED:
      dwin_on_cmd_tool(tval);
      break;

/*case VARADDR_APPLY_RGBLED:
      dwin_on_cmd_tool(tval);
      break;*/      
//END
...
// ADD F.H. 20190326
  myFysTLcd.ftCmdStart(VARADDR_ACTIVE_EXTRUDER_HOTEND);
  myFysTLcd.ftCmdPut16((int16_t)thermalManager.degTargetHotend(0));  
  myFysTLcd.ftCmdSend();
  #if HAS_TEMP_BED
    myFysTLcd.ftCmdStart(VARADDR_ACTIVE_EXTRUDER_BED);
    myFysTLcd.ftCmdPut16((int16_t)thermalManager.degTargetBed());
    myFysTLcd.ftCmdSend();
  #endif
